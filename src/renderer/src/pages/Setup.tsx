...
export default function Setup(): JSX.Element {
  ...
  async function handleGenerate(): Promise<void> {
    ...
    // Duplicates removed and prompts with no trust word, reported by the
    // generator. Persona warnings are appended to these rather than replacing
    // them, so nothing gets dropped when both stages have something to say.
    const baseWarnings = genResult.warnings ?? []
    ...
    if (selectedPersonas.length > 0) {
      ...
      if (personaResult.success) {
        ...
        setWarnings([...baseWarnings, ...(personaResult.warnings ?? [])])
      } else {
